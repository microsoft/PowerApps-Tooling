...
public class StrongTypedStringTests
{
        ...
        where TSelf : ITypedString<TSelf>
    {
        foreach (var input in invalidInputs)
        {
            ...
            TSelf.TryParse(input, out var result2).Should().BeFalse();
            ...
            if (input is null)
            {
                ...
                CreateAction(() => TSelf.Parse(input!)).Should().Throw<ArgumentNullException>();
            }
            else
            {
                CreateAction(() => TSelf.Parse(input, provider: null)).Should().Throw<FormatException>();
                CreateAction(() => TSelf.Parse(input)).Should().Throw<FormatException>();
            }
        }
    }
        ...
        where TSelf : ITypedString<TSelf>
    {
        foreach (var input in validInputs)
        {
            ...
            TSelf.TryParse(input, out var result2).Should().BeTrue();
            ...
            TSelf.Parse(input, provider: null).Should().NotBeNull()
                .And.BeOfType<TSelf>()
                .Which.Value.Should().Be(input);
            TSelf.Parse(input).Should().NotBeNull()
                .And.BeOfType<TSelf>()
                .Which.Value.Should().Be(input);
        }
    }
