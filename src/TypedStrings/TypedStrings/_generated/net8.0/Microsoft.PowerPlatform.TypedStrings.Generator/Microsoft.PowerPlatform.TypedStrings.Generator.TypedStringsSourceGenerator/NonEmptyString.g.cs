...
namespace Microsoft.PowerPlatform.TypedStrings
{
    ...
    public partial record NonEmptyString : global::Microsoft.PowerPlatform.TypedStrings.ITypedString<NonEmptyString>
    {
        ...
        private string GetDebuggerDisplay => $"{nameof(NonEmptyString)} {{ Value = {Value} }}";
        ...
        public static NonEmptyString Parse(string value, IFormatProvider? provider)
        {
            _ = value ?? throw new ArgumentNullException(nameof(value));

            if (!TryParse(value, provider, out var parsed))
            {
                throw new FormatException($"Invalid string format for a NonEmptyString.");
            }

            return parsed;
        }

        public static bool TryParse(string? value, IFormatProvider? provider, [NotNullWhen(true)] out NonEmptyString? result)
        {
            if (!TryValidate(value, out var validated))
            {
                result = default;
