...
namespace PAModelTests
{
    ...
    public class GroupControlRecreationTest
    {
        ...
        [DataTestMethod]
        [DataRow("GroupControlTest_NoChildren.msapp")]
        public void TestGroupControlsState_Empty2(string filename)
        {
            ...
            using (var tempDir = new TempDir())
            {
                ...
                errors = msapp.SaveToSources(outSrcDir, path);
            }

            Assert.IsTrue(errors.ToString().Contains("Warning PA2002: Validation issue: Group control state is empty for Group1"));
            Assert.IsTrue(errors.ToString().Contains("Error   PA3013: Property Value Changed: TopParent.Children[0].GroupedControlsKey"));
        }
    }    
}
