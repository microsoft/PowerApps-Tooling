...
{
    [TestMethod]
    [DataRow(@"_TestData/ValidYaml/Screen-with-name.fx.yaml", typeof(Screen), "http://microsoft.com/appmagic/screen",
        "My Power Apps Screen", 0, 0)]
    [DataRow(@"_TestData/ValidYaml/Screen-with-controls.fx.yaml", typeof(Screen), "http://microsoft.com/appmagic/screen",
        "Screen 1", 2, 2)]
    [DataRow(@"_TestData/ValidYaml/Screen/with-two-properties.fx.yaml", typeof(Screen), "http://microsoft.com/appmagic/screen",
        "Hello", 2, 0)]
    [DataRow(@"_TestData/ValidYaml/Screen/with-properties-and-controls.fx.yaml", typeof(Screen), "http://microsoft.com/appmagic/screen",
        "Screen with two properties and two controls", 2, 2)]
    [DataRow(@"_TestData/ValidYaml/Screen/with-properties-and-nested-controls.fx.yaml", typeof(Screen), "http://microsoft.com/appmagic/screen",
    ...
    public void RoundTrip_ValidYaml(string path, Type rootType, string expectedTemplateId, string expectedName, int expectedPropsCount, int expectedControlCount)
    {
        ...
        var controlObj = deserializer.Deserialize(yamlReader, rootType);
